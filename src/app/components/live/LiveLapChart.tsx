...
const LiveLapChartGraph: React.FC<LapChartProps> = ({ lapsData, positions, drivers }) => {
    ...
    const setLaps = () => {
        ...
        for (let i = 0; i < positions.length; i++) {
            let currentDriverLapData: DriverLapData = { driver: drivers[positions[i]], laps: [] };
            let currentDriverLapNumberData: DriverLapData = { driver: drivers[positions[i]], laps: [] };
            for (let j = 0; j < lapsData[positions[i]].length; j++) {
                if (j > 0) {
                    if (lapsData[positions[i]][j].stint != lapsData[positions[i]][j - 1].stint) {
                        if (currentDriverLapData.laps.length != 0) {
                            lineTyreAgeData.push(currentDriverLapData);
                            currentDriverLapData = { driver: drivers[positions[i]], laps: [] };
                        }
                    }
                }
                if (lapsData[positions[i]][j].isChecked) {
                    currentDriverLapData.laps.push(lapsData[positions[i]][j]);
                    currentDriverLapNumberData.laps.push(lapsData[positions[i]][j]);
                    if (maxLapTime < lapsData[positions[i]][j].lapTime) {
                        maxLapTime = lapsData[positions[i]][j].lapTime;
                    }
                    if (minLapTime > lapsData[positions[i]][j].lapTime) {
                        minLapTime = lapsData[positions[i]][j].lapTime;
                    }

                    if (maxLapNumber < lapsData[positions[i]][j].lapNumber) {
                        maxLapNumber = lapsData[positions[i]][j].lapNumber;
                    }
                    if (minLapNumber > lapsData[positions[i]][j].lapNumber) {
                        minLapNumber = lapsData[positions[i]][j].lapNumber;
                    }

                    if (maxTyreAge < lapsData[positions[i]][j].tyreAge) {
                        maxTyreAge = lapsData[positions[i]][j].tyreAge;
                    }
                    if (minTyreAge > lapsData[positions[i]][j].tyreAge) {
                        minTyreAge = lapsData[positions[i]][j].tyreAge;
                    }
                }
                // else {
                //     if (currentDriverLapData.laps.length != 0) {
            ...
            if (currentDriverLapNumberData.laps.length != 0) {
                ...
                legendData.push({ driverName: drivers[positions[i]].driver, teamColour: drivers[positions[i]].teamColour, isDashed: legendData.map((x) => x.teamColour).includes(drivers[positions[i]].teamColour), position: i + 1 });
            }
        }
        ...
        if (active && payload && payload.length && label) {
            ...
            for (let i = 0; i < lineDataLapNumber.length; i++) {
                for (let j = 0; j < lineDataLapNumber[i].laps.length; j++) {
                    if ((xAxisType == "lap number" ? lineDataLapNumber[i].laps[j].lapNumber : lineDataLapNumber[i].laps[j].tyreAge) == label) {
                        ...
                        return (
                            <div key={index} style={{ marginBottom: "5px" }}>
                                ...
                                <p style={{ margin: "2px 0" }}>
                                    Tyre: <span style={{ color: compoundColours[entry.lap.compound], fontWeight: "600" }}>{entry.lap.compound}{xAxisType == "lap number" ? ` (${entry.lap.tyreAge})` : ""}</span>
                                </p>
                            </div>
                        );
                    })}
            ...
            <Box flexGrow={1} height="0" minHeight={0}>
                <ResponsiveContainer width="100%" height="700px">
                    <LineChart key={graphKey}>
                        ...
                        <XAxis type="number" dataKey={xAxisType == "lap number" ? "lapNumber" : "tyreAge"} domain={xAxisType == "lap number" ? [minLapNumber - 1, maxLapNumber + 1] : [minTyreAge - 1, maxTyreAge + 1]} tickCount={xAxisType == "lap number" ? maxLapNumber - minLapNumber + 3 : maxTyreAge - minTyreAge + 3} tick={{ fill: 'white' }} />
                        ...
                        {<Tooltip content={<CustomTooltip />} />}
                        {
                            xAxisType == "lap number" ?
                                lineDataLapNumber.map((driverData) => (
                                    <Line
                                        key={driverData.driver.driver + driverData.laps[0].lapNumber.toString()}
                                        ...
                                        strokeWidth={3}
                                        strokeDasharray={driverDataLegend[driverDataLegend.map((x) => x.driverName).indexOf(driverData.driver.driver)].isDashed ? "8, 8" : "0"}
                                        dot={(props) => {
                                            ...
                                            return (
                                                <circle
                                                    key={`${driverData.driver.driver}-${payload.lapNumber}`}
                                                    cx={cx}
                                                    cy={cy}
...
export default LiveLapChartGraph;
